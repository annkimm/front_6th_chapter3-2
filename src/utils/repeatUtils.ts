...
export function generateRepeatEvents(baseEvent: EventForm, endDate?: string): Event[] {
  // 1단계: 반복이 없으면 빈 배열
  ...
  let loopCount = 0; // 무한루프 방지
  
  while (currentDate <= new Date(finalEndDate) && loopCount < 50) {
    loopCount++;
    
    console.log(`루프 ${loopCount}: ${formatDate(currentDate)} (${currentDate.getFullYear()}년 ${currentDate.getMonth()+1}월 ${currentDate.getDate()}일)`);
    ...
    if (type === 'yearly' && isLeapDayEvent) {
      ...
      console.log(`월별 체크: ${currentDate.getMonth()+1}월 최대일수=${daysInCurrentMonth}, 목표일=${targetDay}, 현재일=${currentDate.getDate()}`);
      console.log(`목표일 존재: ${targetDayExists}, 날짜 일치: ${dateMatches}`);
      
      shouldCreateEvent = targetDayExists && dateMatches;
    }
    
    console.log(`이벤트 생성 조건: ${shouldCreateEvent} (목표일: ${targetDay}, 현재일: ${currentDate.getDate()})`);
    ...
    if (shouldCreateEvent) {
      ...
      console.log(`이벤트 생성됨: ${formatDate(currentDate)}`);
    }
    
    // 5단계: 다음 날짜로 이동
    console.log('다음 날짜로 이동 전:', formatDate(currentDate));
    moveToNextDate(currentDate, type, baseEvent.repeat.interval, targetDay, targetMonth);
    console.log('다음 날짜로 이동 후:', formatDate(currentDate));
  }
  
  console.log(`총 ${events.length}개 이벤트 생성됨`);
  return events;
}

function moveToNextDate(date: Date, type: RepeatType, interval: number, targetDay: number, targetMonth?: number) {
  switch (type) {
    case 'daily':
